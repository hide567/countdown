 ...
 * 定期タスクの設定
 */
function ecm_schedule_cron_jobs() {
 ...
 * 定期タスクの解除
 */
function ecm_unschedule_cron_jobs() {
...
/**
 * ショートコードのレンダリングエラーをログに記録
 *
 * @param string $shortcode ショートコード名
 * @param string $error_message エラーメッセージ
 * @param array $atts ショートコード属性
 */
function ecm_log_shortcode_error($shortcode, $error_message, $atts = array()) {
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log(sprintf(
            'ECM Shortcode Error [%s]: %s - Attributes: %s',
            $shortcode,
            $error_message,
            json_encode($atts)
        ));
